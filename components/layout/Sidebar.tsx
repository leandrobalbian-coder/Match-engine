'use client'
import { LayoutDashboard, Users, Building2, UserCog, Sparkles, MapPin } from 'lucide-react'
...
export function Sidebar({
  ...
  const items = [
    ...
    { icon: Building2, label: 'Espacios' },
    { icon: UserCog, label: 'Brokers' },
    { icon: Sparkles, label: 'MatchAgent', active: true },
  ]
  return (
    <aside
      className={cn(
    ...
    >
        ...
        {items.map((it, idx) => {
          ...
          return (
            <div
              key={it.label}
              className={cn(
                'group relative flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-semibold cursor-pointer transition-colors',
                it.active
      ...
      <div className="px-3 pt-3 pb-4 border-t border-white/5">
        <div className="text-[10px] uppercase tracking-wider text-white/40 font-bold mb-2 px-2">
          Demo · pantalla {Math.max(current, 1)} de 7
        </div>
        ...
        <div className="mt-3 px-2 flex items-center gap-1.5 text-[10px] text-white/40">
